...
class Playground(var items: Map[Position, Item]) {
  ...
  def move(from: Position, to: Position): Move = {
    ...
    (fromItem.get, toItem.get) match {
      case (player: Player, way: Way) =>
        validMove; PlayerToWay()
      case (player: Player, portal: Portal) =>
        validMove; PlayerToPortal()
      case (bot: Bot, way: Way) =>
        validMove; BotToWay()
      case (bot: Bot, player: Player) =>
        validMove; BotToPlayer()
      case (player: Player, bot: Bot) =>
        validMove; BotToPlayer()
  ...
  def load(file: String): Playground = {
    ...
    arr.foreach(input =>
      input match {
        case '|' =>
          items += (new Position(x, y) -> new Wall); x += 1
        case '1' =>
          items += (new Position(x, y) -> new Player("1", NoMove())); x += 1
        case ' ' =>
          items += (new Position(x, y) -> new Way); x += 1
        case 'P' =>
          items += (new Position(x, y) -> new Portal); x += 1
        case 'B' =>
          items += (new Position(x, y) -> new Bot(Left())); x += 1
        case '\n' =>
          x = 0; y += 1
        case '\r' => None
