...
class FireSprite extends TerrainSprite {
  ...
  def enterAnimation(player : Player) = sprite.update(0, if (player.isInstanceOf[Bot]) -32 else 0)
  def leaveAnimation(player : Player) = sprite.update(-32, if (player.isInstanceOf[Bot]) -32 else 0)
}
